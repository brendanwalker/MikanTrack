...
static int runFusionTest(const TestArgs& args)
{
	...
	// reprojection residual must veto the pairing.
	{
		...
		//
		// Depth error from a stereo pair goes as 1/sin(parallax), and it
		// lands on the small out-of-plane finger geometry that decides the
		// palmar side - so picking a narrow pair does not merely add noise,
		// it flips the palm frame 180 degrees. Here camC sits where it sees
		// the palm nearly face-on (high blend weight) but subtends a much
		// narrower angle with camB than camA does; ranking by per-camera
		// weight takes that bait, so the assertion is that the WIDEST pair
		// wins anyway.
		{
			const glm::vec3 camCPos= palmTruth + glm::vec3(0.f, -0.30f, 0.62f);

			auto lowerPresence= [](CameraFrameResult& camera, float presence) {
				camera.result.hands[(int)eHandSide::Right].presence= presence;
				camera.result.poses[(int)eHandSide::Right].presence= presence;
			};

			auto camAWeak= makeStereoResult(0, camAPos, worldHand, worldHand, palmTruth);
			lowerPresence(camAWeak, 0.7f); // the wide-baseline camera is the weaker one
			auto camBTri= makeStereoResult(1, camBPos, worldHand, worldHand, palmTruth);
			auto camCTri= makeStereoResult(2, camCPos, worldHand, worldHand, palmTruth);

			HandFusion pairFusion;
			pairFusion.configure(fusionConfig);
			TrackingFrameResult pairFused;
			pairFusion.fuse({&camAWeak, &camBTri, &camCTri}, now, pairFused);

			auto parallaxDeg= [&](const glm::vec3& a, const glm::vec3& b) {
				return glm::degrees(acosf(std::clamp(
					glm::dot(glm::normalize(palmTruth - a), glm::normalize(palmTruth - b)), -1.f, 1.f)));
			};
			const float parallaxAB= parallaxDeg(camAPos, camBPos);
			const float parallaxAC= parallaxDeg(camAPos, camCPos);
			const float parallaxBC= parallaxDeg(camBPos, camCPos);
			const float widest= std::max(parallaxAB, std::max(parallaxAC, parallaxBC));

			const FusionDiagnostics& pairDiag= pairFusion.getLastDiagnostics();
			const bool bHaveCluster= !pairDiag.clusters.empty() && pairDiag.clusters[0].triangulated;
			const int chosenA= bHaveCluster ? pairDiag.clusters[0].triCameraA : -1;
			const int chosenB= bHaveCluster ? pairDiag.clusters[0].triCameraB : -1;
			const float chosenParallax= bHaveCluster ? pairDiag.clusters[0].triParallaxDeg : 0.f;

			MIKAN_LOG_INFO("test-fusion")
				<< "(r) pair choice: cameras " << chosenA << "+" << chosenB << " at "
				<< chosenParallax << " deg (available " << parallaxAB << ", " << parallaxAC
				<< ", " << parallaxBC << ")";

			// The reported parallax is measured to the cluster anchor (a
			// monocular palm estimate), so it lands near the true angle
			// rather than on it - the substantive claim is that the pair
			// chosen is the wide one
			if (!bHaveCluster || (chosenA + chosenB) != 1 || // cameras 0 and 1
				chosenParallax <= parallaxAC || chosenParallax <= parallaxBC ||
				widest != parallaxAB)
			{
				MIKAN_LOG_ERROR("test-fusion")
					<< "(r) FAILED: the widest-parallax pair must be triangulated";
				result= 1;
			}
		}
