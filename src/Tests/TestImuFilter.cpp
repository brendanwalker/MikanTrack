...
static int runImuFilterTest(const TestArgs& args)
{
	...
	// (p) A DECISIVE contradiction of the remembered palmar side has to
	// persist before it is believed.
	//
	// (m) covers weak evidence, where continuity already won. This covers
	// the case that broke the three-camera rig: a short-baseline stereo pair
	// reconstructs a straight-fingered hand with its curl inverted, so the
	// evidence is decisively WRONG rather than merely weak and blew straight
	// through continuity, flipping the palm frame 180 degrees. A brief burst
	// must be ignored; a sustained one must still be adopted, or a memory
	// that started out wrong could never recover.
	{
		// Curl deep enough to score well past the decisive threshold, so the
		// sign of curlSign IS the sign of the evidence. The step index nudges
		// the landmarks, because the count advances once per distinct
		// observation and real landmarks never repeat exactly.
		auto buildCurledHand= [](float curlSign, int step) {
			std::array<glm::vec3, HAND_LANDMARK_COUNT> points{};
			points[(int)eHandLandmark::WRIST]= glm::vec3(0.f, 0.f, 0.f);
			const float spread[FINGER_COUNT]= {0.035f, 0.02f, 0.f, -0.018f, -0.032f};
			for (int finger= 0; finger < FINGER_COUNT; ++finger)
			{
				...
				for (int joint= 1; joint < 4; ++joint)
				{
					const float along= 0.022f * (float)joint + 0.00002f * (float)step;
					const float curl= curlSign * 0.006f * (float)(joint * joint);
					points[joints[joint]]= base + glm::vec3(along, 0.f, curl);
				}
			}
			return points;
		...
		// Twice per step, as a real frame evaluates it (palm frame, then
		// angles): that is one observation and must count once
		int stepIndex= 0;
		auto palmZ= [&](float curlSign, HandPoseModel::PalmarSideMemory& memory) {
			const std::array<glm::vec3, HAND_LANDMARK_COUNT> points= buildCurledHand(curlSign, stepIndex++);
			HandPoseModel::computePalmFrame(points, eHandSide::Left, &memory);
			return glm::vec3(HandPoseModel::computePalmFrame(points, eHandSide::Left, &memory)[2]);
		};

		HandPoseModel::PalmarSideMemory memory;
		const glm::vec3 established= palmZ(1.f, memory);

		// Four contradicting frames, then the truth returns
		int flipsDuringBurst= 0;
		for (int step= 0; step < 4; ++step)
			if (glm::dot(palmZ(-1.f, memory), established) < 0.f)
				flipsDuringBurst++;
		const bool bRecovered= glm::dot(palmZ(1.f, memory), established) > 0.f;

		// Sustained contradiction: the side must eventually be adopted, or a
		// memory seeded wrong at acquisition would be stuck forever
		int framesToFlip= 0;
		bool bEventuallyFlipped= false;
		for (int step= 0; step < 30 && !bEventuallyFlipped; ++step)
		{
			framesToFlip++;
			bEventuallyFlipped= glm::dot(palmZ(-1.f, memory), established) < 0.f;
		}

		MIKAN_LOG_INFO("test-imufilter")
			<< "(p) flip hysteresis: " << flipsDuringBurst << " flips in a 4-frame burst, recovered="
			<< bRecovered << ", sustained contradiction adopted after " << framesToFlip << " frames";

		if (flipsDuringBurst != 0 || !bRecovered || !bEventuallyFlipped)
		{
			MIKAN_LOG_ERROR("test-imufilter")
