...
- `src/App`: process orchestration and the record/replay/diagnostics machinery. `App` (`App.h`) owns startup/shutdown, the SDL window and GL context, and the ~90 Hz main loop (`FrameTimer(11)` in `App::exec`). `VisionThread` is the hub of the app: the inference thread that drains cameras, runs the ML pipelines, fuses, and streams (see Frame anatomy below). `AppConfig` is the persisted settings model. `TrackingRecording`/`TrackingRecorder`/`TrackingReplay`/`FrameRecorder`/`DiagnosticDump`/`TrackingJson` are the recording and diagnostics inventory (see below).
...
`AppConfig` (`src/App/AppConfig.h`/`.cpp`) is the persisted settings model, stored as JSON at `%APPDATA%/MikanMediaPipe/config.json` (`AppConfig::getConfigFilePath`). Edits call `markDirty()`; `updateAutoSave` on the main loop saves at most once per cooldown (`k_autoSaveCooldownSeconds`, 3 s). `toJsonString`/`loadFromJsonString` serialize the same schema in memory, which is how recording headers snapshot the live config and how replay reconstructs it.
